    ...
    return(
        ...
        pt-8 max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm">
            ...
            <div className="group relative">
                <label htmlFor="image">
                    <img src={image ? URL.createObjectURL(image) : user?.image} alt="" className="h-9 
                    md:h-14 w-9 md:w-14 rounded-full mx-auto" />
            ...
            <div className="w-full">
                {ownerMenuLinks.map((link, index)=>(
                    <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path
                        ...
                        <span className="mad-md:hidden">{link.name}</span>
